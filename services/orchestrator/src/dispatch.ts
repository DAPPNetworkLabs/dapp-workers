...
export async function dispatchService(id, dockerImage, ipfsInput, args): Promise<any> {
  ...
  if(process.env.DAPP_WORKERS_K8S) {
    ...
    const cmd = `${generatedArgs.join(' ')} WORKERS_SERVICE_NAME=${imageName}-${id} IPFS_HOST=${process.env.IPFS_HOST} envsubst < /dapp-workers/k8s/common/images/${imageName}.yaml | kubectl apply -f -`
    try {
      // console.log(cmd);
